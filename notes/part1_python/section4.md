...
![四种容器一览](section4/containers-overview.png)
...
![zip 与 enumerate](section4/zip-enumerate.png)
...
`map` 和 `filter` 是两个很常用的加工列表的工具。它们第一个参数要传**一个"加工动作"**——别慌，**不需要自己写**，用我们已经见过的内置工具就行（比如第 2 节学的 `int`、`str`）。

**`map(加工动作, 列表)`：对每个元素套用同一个动作**
...
读法：`map(str, nums)` 就是"拿 `str` 这个转换工具，挨个处理 nums 的每个元素"。注意 `map` 里写的是 `str` 而**不是** `str()`——给的是"工具本身"（一个加工动作），不是用它加工一次的结果。

**`filter(判断动作, 列表)`：判断为真的元素才留下**
...
实际写代码时**优先用列表推导式**，更符合 Python 风格；但 `map`/`filter` 在阅读别人代码、以及像 `map(int, ...)` 这种"直接套用现成工具"的场景里很常见，要会用。到第 5 节学会自己定义加工规则后，它们会更灵活。
...
![列表按位置找，字典按名字找](section4/dict-lookup.png)
...
**字典里还能再装字典（或列表）——这就是嵌套。** 值可以是任意东西，自然也可以是另一个容器：

```python
student = {
    "name": "小明",
    "age": 18,
    "scores": {"语文": 85, "数学": 92}     # 值又是一个字典
}
```

嵌套的取值：**把 `[]` 连着写，从左到右一层层往里取**：
...
print(student["scores"]["数学"])     # 92
```

拆成两步看，其实每一步都是已经学过的操作：
...
scores = student["scores"]     # 第一步：取出里面的字典 {"语文": 85, "数学": 92}
print(scores["数学"])          # 第二步：再按"数学"取值 → 92
```

改嵌套里的值也一样，一路取到那一层再赋值：
...
student["scores"]["数学"] = 95     # 只改里层的"数学"，其他不动
```

嵌套也想用 `get()` 安全取值？**一层层分开 `get`，每层的默认值给一个空字典 `{}`**，这样下一层永远有东西可取：
...
print(student.get("scores", {}).get("数学"))          # 92
print(student.get("scores", {}).get("英语", "暂无"))  # 暂无
print(student.get("hobby", {}).get("数学"))           # None：外层没有也不报错
```

注意为什么不能直接连着写 `student.get("scores").get("数学")`：万一 `"scores"` 这个键不存在，第一个 `get` 返回的是 `None`，再对 `None` 取值就直接报错了。中间垫一个 `{}` 就是防这个的。

4.6 的通讯录实战就是嵌套的典型用法：**列表里装字典**（`contacts[0]["phone"]`——先按索引取出一个字典，再按键取值）。
...
![四种容器怎么选](section4/container-choice.png)

**但真实的数据很少是"纯"的——更多时候是几种容器组合着用。** 下面三种组合最常用，看懂了它们，前面学的嵌套取值就全串起来了：

**组合一：列表装字典——一串"有字段"的记录**（4.6 通讯录就是这个结构）
...
print(students[1]["name"])     # 小红：先按索引取出第 1 个字典，再按键取值
```

列表管"顺序"（第几个学生），字典管"字段"（每个学生叫什么名字）。这是实际代码里出现频率最高的组合。
...
print(groups["一班"][0])       # 小明：先按班取出名单列表，再按索引取人
```

字典管"分类"（哪个班），列表管"这一类里的一串"（班里有哪些人）。

**组合三：集合给列表去重，去完接着当列表用**

```python
nums = [1, 2, 2, 3, 3, 3]
unique = list(set(nums))       # 先用集合去重，再转回列表
print(unique[0])               # 1：转回列表才能继续用索引、切片
```

集合不负责"顺序"，所以去重之后通常要转回列表接着干活——这个 `list(set(...))` 写法会反复见到。

**元组什么时候上场？** 组合里需要"一小包固定数据"时：比如一串坐标点 `[(1, 2), (3, 4), (5, 6)]`——每个点永远是两个数、不会改，用元组装点；点有很多个、会增减，用列表装这些元组。

记住一句话：**容器是积木，单个容器选"形状"，真实数据靠组合。**
...
**为什么列表改一个、另一个跟着变；数字却不会？**

### 4.5.2　拆开看：b = a 到底干了什么

回忆之前的内存模型：**变量名只是"指向对象的标签（对象在堆里）"，赋值 `=` 是"贴标签"，不是"复制内容"。**
...
而数字为什么没事？因为**数字是"不可变"的**——`b = b + 1` 并没有"修改"原来那个 100，而是**新建**了一个对象 101，让 `b` 改指向它。`a` 还指着原来的 100，纹丝不动。
...
![浅拷贝的坑](./section4/shallow-copy.png)
...
![浅拷贝的坑](./section4/shallow-copy-problem.png)
...
a = [[1, 2], [3, [4, 5]]]      # 三层嵌套：内层里还套着列表
b = copy.deepcopy(a)

b[0].append(99)                # 改第 2 层
b[1][1].append(6)              # 改第 3 层（浅拷贝管不到的地方）

print(a)                       # [[1, 2], [3, [4, 5]]]      —— 两层都纹丝不动
print(b)                       # [[1, 2, 99], [3, [4, 5, 6]]]

print(id(a[0]) == id(b[0]))         # False：第 2 层是新对象
print(id(a[1][1]) == id(b[1][1]))   # False：第 3 层也是新对象

```

![深拷贝：连内层也复制](./section4/deep-copy.png)
...
- 以后（第 5 节）还会遇到这个坑的升级版：把列表交给另一段代码去处理，那边改、这边也变——因为两边指的是同一个对象。
...
![通讯录的结构：列表里装着字典](section4/contacts-structure.png)
...
- 列表 `list`：有序可变，`append`/`insert`/按索引改、`remove`/`pop` 删、`in`/`index` 查，`for` 遍历（`enumerate` 带序号、`zip` 配对），推导式一行造新列表；`map`/`filter` 传内置工具即可用，返回值要 `list()` 才能看到。
